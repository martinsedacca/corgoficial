-- This migration updates the RPC function to correctly handle the 'ao' column,
-- which is required by the prescription_items table, fixing the not-null constraint violation.
...
BEGIN
    ...
    LOOP
        IF v_item->>'id' IS NOT NULL THEN -- Update existing item
            UPDATE public.prescription_items
            SET 
                practice_id = (v_item->>'practice_id')::uuid,
                ao = (v_item->>'ao')::text -- Add the 'ao' field to the update
            WHERE id = (v_item->>'id')::uuid;
        ELSE -- Insert new item
            INSERT INTO public.prescription_items (prescription_id, practice_id, ao)
            VALUES (p_prescription_id, (v_item->>'practice_id')::uuid, (v_item->>'ao')::text); -- Add the 'ao' field to the insert
        END IF;
    END LOOP;
