-- This migration fixes the user attribution in logs by passing the user's email
-- directly to the RPC function, instead of relying on auth context within a SECURITY DEFINER.

CREATE OR REPLACE FUNCTION public.update_prescription_and_items(
    p_prescription_id uuid,
    p_prescription_data jsonb,
    p_items_data jsonb,
    p_user_email text -- New parameter for user email
)
RETURNS void AS $$
DECLARE
    ...
    v_new_item_ids uuid[];
BEGIN
    -- (The user info block is no longer needed)
    ...
    LOOP
        IF v_item->>'id' IS NOT NULL AND (v_item->>'id')::uuid = ANY(v_old_item_ids) THEN
            UPDATE public.prescription_items
            SET ao = (v_item->>'ao')::text
            WHERE id = (v_item->>'id')::uuid AND ao IS DISTINCT FROM (v_item->>'ao')::text;
        ELSE
    ...
    IF v_old_prescription IS DISTINCT FROM v_new_prescription OR v_old_items IS DISTINCT FROM v_new_items THEN
        INSERT INTO public.logs (user_email, action, entity, entity_id, entity_description, previous_data, new_data, ip_address)
        VALUES (
            p_user_email, -- Use the email passed from the frontend
            'UPDATE',
            'prescriptions',
            p_prescription_id,
